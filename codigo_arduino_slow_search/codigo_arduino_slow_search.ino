...
void loop() {
  ...
  float dd= ud.convert(md, Ultrasonic::CM);
	...
	while(digitalRead(sensor1) == 0 && digitalRead(sensor2) == 0){	//verifica se esta dentro da borda
		if(dd>20 && de>20)slow();
		else if(dd<=20 && de>dd)direita();
		else if(dd>de && de<=20)esquerda();
		else if(dd==de && (de<=20¦¦ dd<=20))ataque();	
